...
void ptpd_netif_set_phase_transition(uint32_t phase, int port)
{
	...
	for (i=0; i< ARRAY_SIZE(socks[port]); ++i) {
		socks[port][i]->phase_transition_rx = phase;
	}
}
					     ...
					     int udp_or_raw, int udpport, int port)
{
	...
	sock->phase_transition_rx = pstate.t2_phase_transition;
	sock->phase_transition_tx = 0;
	sock->dmtd_phase_rx = pstate.phase_rx_val;
	sock->dmtd_phase_tx = pstate.phase_tx_val;
...
void ptpd_netif_linearize_tx_timestamp(struct wr_timestamp *ts,
				       int32_t dmtd_phase_tx,
				       int cntr_ahead, int transition_point,
				       int clock_period)
{
	int nsec_f, nsec_r;

	ts->raw_phase =  dmtd_phase_tx;
...
/* Calculate the ncounter value for both timestamps. The rising edge one
   is just the HW register */
	nsec_r = ts->nsec;
/* The falling edge TS is the rising - 1 thick
    if the "rising counter ahead" bit is set. */
			...
			size_t data_length, struct wr_timestamp *rx_timestamp, int port)
{
	...
	if (rx_timestamp) {
		...
		spll_read_ptracker(2*port, &rx_timestamp->raw_phase, NULL);
		      ...
		      size_t data_length, struct wr_timestamp *tx_timestamp, int port)
{
	...
	if (tx_timestamp) {
		tx_timestamp->raw_nsec = hwts.nsec;
		tx_timestamp->raw_ahead = hwts.ahead;
		spll_read_ptracker(2*port+1, &tx_timestamp->raw_phase, NULL);

		tx_timestamp->sec = hwts.sec;
		tx_timestamp->nsec = hwts.nsec;
		tx_timestamp->phase = 0;
		tx_timestamp->correct = hwts.valid;
		ptpd_netif_linearize_tx_timestamp(tx_timestamp,
						  tx_timestamp->raw_phase,
						  hwts.ahead,
						  s->phase_transition_tx,
						  REF_CLOCK_PERIOD_PS);
	}
	return rval;
}
