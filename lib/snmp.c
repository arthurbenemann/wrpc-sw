...
static char init_script_ptp_config[32];
...
static uint8_t oid_wrpcInitScriptConfigSfpPn[] =        {2,0};
...
static struct snmp_oid oid_array_wrpcInitScriptConfigGroup[] = {
	...
	OID_FIELD_VAR(   oid_wrpcInitScriptConfigSfpPn,     get_p,        set_p,                  ASN_OCTET_STR, &init_script_ptp_config),
	{ 0, }
};
...
static int set_init_script_config(uint8_t *buf, struct snmp_oid *obj)
{
	...
	snmp_verbose("%s: init_script_ptp_config: %s\n", __func__,
		     init_script_ptp_config);
	return ret;
}
