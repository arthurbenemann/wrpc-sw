...
\def \wrpcrelease {wrpc-v4.2-sis8300ku}
...
\textit{wrpc-sw}) are tagged with \texttt{wrpc-v4.2-sis8300ku} tag. Other tools used to
...
\item \href{http://www.ohwr.org/projects/fasec}{FASEC board} +
  \href{http://www.ohwr.org/projects/fmc-dio-5chttla}{FMC DIO card}
\item \href{https://www.struck.de/sis8300-ku.html}{Struck SIS8300-KU digitizer board}
\end{itemize}
...
Depending on whether you want to run the WRPC on Xilinx (e.g. SPEC, SVEC, FASEC, SIS8300-KU boards) or
...
repository for the v4.2 release. The set of commands below clones the WR PTP Core
...
$ git checkout wrpc-v4.2-sis8300ku
$ git submodule init
...
like to synthesize the reference design for Kintex Ultrascale (SIS8300-KU board), you need to first
re-create a Vivado project using the provided TCL script (Vivado projects are not really movable between different machines):

\begin{itemize}
\item Launch Vivado 2018.2
\item From the tcl shell, chdir to \texttt{<your\_location>/wr-cores/top/sis8300ku\_ref\_design}
\item Run the \texttt{project.tcl} script by typing \texttt{source project.tcl}
\end{itemize}

If everything goes well, Vivado will create a fresh reference design project with a top level design block diagram instantiating the SIS8300KU WR Core board support module, the Xilinx PCI Express endpoint core and an AXI interconnect. Click "Generate Bitstream" and wait for some minutes to get the bitstream.
...
repository tagged with \texttt{wrpc-v4.2-sis8300ku} tag. The commands in the listing below
...
the defaults for wrpc-v4.2 release bitstream available on \textit{ohwr.org},
...
startup. If you run WRPC release v4.2 or higher, the default LM32 software
...
The default WR PTP Core v4.2 release firmware comes with the following VIDs:
\begin{itemize}
...
\appendix
\clearpage
\section{SIS8300-KU specific information}
\label{SIS8300-KU specific information}

\subsection{Accessing WRPC serial console}
The SIS8300-KU console support is done in different way than in the SPEC and it needs a different tool. There is a Python script available in the WRPC repository, allowing to access the serial console (VUART) for diagnostic or monitoring purposes:

\begin{lstlisting}
$ cd tools/sis8300ku
$ make
$ ./sis83k_wr_refdesign_vuart.py
\end{lstlisting}
