...
available at \url{https://ohwr.org/project/wr-cores/wikis/Documents/} for officially
...
\item \url{https://ohwr.org/project/wr-cores/wikis/Documents/}
...
\item \url{https://ohwr.org/project/wr-cores.git}
...
\item \url{https://ohwr.org/project/hdl-make.git}
...
can be chosen:
\begin{itemize}
\item \href{https://ohwr.org/project/spec}{SPEC PCIe board} +
  \href{https://ohwr.org/project/fmc-dio-5chttla}{FMC DIO card} + PC
  computer running Linux
\item \href{https://ohwr.org/project/svec}{SVEC VME board} + VME crate with
  a single board computer running Linux\footnote{\label{note_a20}In our test setup
    we used MEN A20 board}
\item \href{https://ohwr.org/project/vfc-hd}{VFC-HD VME board} +
  \href{https://ohwr.org/project/fmc-dio-5chttla}{FMC DIO card} +
  VME crate with a single board computer running Linux\footref{note_a20}
\item \href{https://ohwr.org/project/fasec}{FASEC board} +
  \href{https://ohwr.org/project/fmc-dio-5chttla}{FMC DIO card}
\end{itemize}
...
\begin{itemize}
  ...
  \item a pair of WR-supported SFP transceivers\footnote{The list of supported
    ...
    \url{https://ohwr.org/project/white-rabbit/wiki/SFP}}
...
\begin{lstlisting}
$ git clone https://ohwr.org/project/spec-sw.git <your_location>/spec-sw
...
\begin{lstlisting}
$ wget https://www.ohwr.org/project/spec-sw/uploads/636e7eaa2e1c3a884e2f9f694fdfd1b9/\
spec-init.bin-2015-09-18
...
(\url{https://ohwr.org/project/white-rabbit/wikis/Calibration}). If you
...
(\url{https://ohwr.org/project/white-rabbit/wikis/Documents/White-Rabbit-calibration-procedure}).\\
...
at: \url{https://ohwr.org/project/white-rabbit/wikis/Documents/White-Rabbit-calibration-procedure}}.
...
\begin{longtable}{  p{7.5cm}  p{7cm} }
  ...
  \code{daclog off} &\\
  \code{daclog <ipaddr> <macaddr>} & sends the \textit{DAC} output and input to
  ...
  \code{init add <cmd>}\hypertarget{cmd init add}{} & adds shell command at
    the end of the initialization script \\
  ...
  \code{insitu <on|off|crtt>} & parameter \textit{on} enables printout of t1-t4
                                timestamps.
                                Parameter \textit{crtt} enables printout of
                                round trip time corrected by deltas.
                                Parameter \textit{off} disables printout of
                                timestamps and round trip times.
                                Executing this command without a parameter
                                toggles between \textit{on} and \textit{off}
                                functionality.
                                Available if \texttt{CONFIG\_INSITU\_CALIB} is
                                set. \\
  \code{insitu run <ch\_start> <ch\_end> <step> <samples>} & starts automatic
                                sweep over number of tunable SFP's channels.
                                From channel \textit{ch\_start} to (including)
                                channel \textit{ch\_end} with a defined
                                \textit{step}.
  ...
  \code{pps} & check status of a PPS generation independent of PPSI state \\

  \code{pps force <on|off>} & enable/disable PPS generation independent of
    PPSI state. It is usefull mostly for testing purposes \\
  ...
  \code{|sync\_int|delay\_req\_int> <val>}
                ...
                \texttt{ptp} subcommands listed below.
                Consider using \texttt{ptp master} and \texttt{ptp start} 
                before using mentioned subcommands.\\
  \code{ptp prio1 <0-255>}    & sets \textit{priority 1} used by PTP \\
  \code{ptp prio2 <0-255>}    & sets \textit{priority 2} used by PTP \\
  \code{ptp domain <0-255>}   & sets \textit{domain numbner} used by PTP \\
  \code{ptp class <1-255>}    & sets \textit{clock class} used by PTP \\
  ...
  \code{ptrack <enable|disable|insitu>} & print status, enable or disable phase
                                          tracking. Parameter \textit{disable}
                                          is useful for demos. Using parameter
                                          \textit{insitu} disables
                                          synchronization for WR. In such case
                                          a device only syntonize to the master.
                                          This command does not affect standard
                                          PTP-mode.
                                          Parameter \textit{insitu} is available
                                          if \texttt{CONFIG\_INSITU\_CALIB} is
                                          set. \\
  ...
  \code{sdb fs <memtype> <baseadr> <param>}\hypertarget{cmd sdb fs}{} & creates
    SDBFS image under
  ...
  \code{sdb fse <memtype> <baseadr> <param>}\hypertarget{cmd sdb fse}{} &
...
\begin{lstlisting}
$ wget https://ohwr.org/project/wr-cores/uploads/6f33b5a8660bd0f2ff7a07526b4484d6/\
sdbfs-flash.bin
\end{lstlisting}
...
\begin{lstlisting}
$ wget https://ohwr.org/project/wr-cores/uploads/9dad995e74a3d49b472b8654833be01a/\
