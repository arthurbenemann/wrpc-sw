...
#define UPDATE_LINK_LEDS_PERIOD 100 /* ms */
#define UPDATE_SFP_DOM_PERIOD 1000 /* ms */

// additional MDIO registers used in fixed phase calibration
...
static timeout_t link_led_blink_tmo;
...
static int pcs_writel(struct hal_port_state * p, uint16_t value, int reg)
{
	struct ifreq ifr;
	uint32_t rv;
...
static struct rts_pll_state tx_fsm_pll_state;


uint64_t get_tics(void)
{
...
static void tx_fsm_init( struct hal_port_state *p )
{
	struct hal_port_tx_setup_state* fsm = p->tx_setup_fsm;
	int i;
	...
	for(i = 0; i < RTS_PLL_CHANNELS; i++)
		tx_fsm_pll_state.channels[i].flags = 0;
}
...
static void update_tx_calibration_file(void)
{
	int i;
	...
	for (i = 0; i < HAL_MAX_PORTS; i++)
		if (ports[i].in_use) {
			...
			snprintf(key_name, sizeof(key_name), "TX_PHASE_PORT%d",
				 ports[i].hw_index);
			cfg_set_int(cfg, key_name,
				    ports[i].tx_setup_fsm->measured_phase);
		}

	pr_info("Writing TX phase calibration data to %s\n",
		calibration_file_name);
	cfg_save(cfg, calibration_file_name);
	cfg_close(cfg);
...
static void load_tx_calibration_file(void)
{
	struct config_file *cfg = cfg_load(calibration_file_name, 0);
	int i;
	...
	for (i = 0; i < HAL_MAX_PORTS; i++)
		if (ports[i].in_use) {
			...
			if (cfg_get_int( cfg, key_name, &value)) {
				printf("cal: %d %d\n", ports[i].hw_index, value);
				ports[i].tx_setup_fsm->cal_saved_phase = value;
				ports[i].tx_setup_fsm->cal_saved_phase_valid = 1;
				ports[i].tx_setup_fsm->cal_file_updated = 1;
			}
		}
...
static int tx_fsm_done_for_all_ports(void)
{
	int i;
	for (i = 0; i < HAL_MAX_PORTS; i++)
...
static int tx_fsm_update( struct hal_port_state *p )
{
	struct hal_port_tx_setup_state* fsm = p->tx_setup_fsm;
	
//	printf("TxFsmUpd state %d\n", fsm->state);

	if( p->hw_index == 0 )
	{
		// update PLL state once for all ports
		rts_get_state(&tx_fsm_pll_state);
	}
	...
	switch( fsm->state )
	{
		case TX_SETUP_STATE_START:
		{
			...
			pcs_writel( p, MDIO_DBG1_RESET_RX | MDIO_DBG1_DMTD_SOURCE_TXOUTCLK, MDIO_DBG1 );
		...
		case TX_SETUP_STATE_RESET_PCS:
		{
			...
			pcs_writel( p, MDIO_DBG1_RESET_TX | MDIO_DBG1_RESET_RX | MDIO_DBG1_DMTD_SOURCE_TXOUTCLK, MDIO_DBG1 );
			shw_udelay(1);
			pcs_writel( p, MDIO_DBG1_RESET_RX | MDIO_DBG1_DMTD_SOURCE_TXOUTCLK, MDIO_DBG1 );
		...
		{
			if( pcs_readl( p, MDIO_DBG0) & MDIO_DBG0_RESET_TX_DONE )
			{
				fsm->attempts++;
		...
		case TX_SETUP_STATE_MEASURE_PHASE:
		{
			...
			int phase = tx_fsm_pll_state.channels[ p->hw_index ].phase_loopback;
		...
		pr_info("TX Calibration: upd port %d phase %d %d after %d "
			"attempts target %d tollerance %d (temp = %d)\n",
			p->hw_index, fsm->measured_phase, fsm->measured_phase,
			fsm->attempts, fsm->expected_phase, fsm->tollerance,
			...
			if( within_range( phase, phase_min, phase_max, 16000 ) )
			{
				...
				//				pr_info("FIX port %d phase %d after %d attempts (temp = %d)\n", p->hw_index, fsm->measured_phase_noavg, fsm->attempts, hal_get_fpga_temperature() );
				rts_enable_ptracker( p->hw_index, 0 );
				rts_enable_ptracker( p->hw_index, 1 );
		...
		case TX_SETUP_VALIDATE:
			...
			fsm->measured_phase = tx_fsm_pll_state.channels[ p->hw_index ].phase_loopback;
...
static void rx_fsm_init( struct hal_port_state *p )
{
...
static int rx_fsm_update( struct hal_port_state *p )
{
	struct hal_port_rx_setup_state* fsm = p->rx_setup_fsm;
	...
	switch( fsm->state )
	{
		...
		case RX_SETUP_VALIDATE:
		{
			...
			if ( tx_fsm_pll_state.channels[p->hw_index].flags & CHAN_PMEAS_READY )
			{
				...
				pcs_writel( p, MDIO_DBG1_RX_ENABLE | MDIO_DBG1_TX_ENABLE | MDIO_DBG1_DMTD_SOURCE_RXRECCLK, MDIO_DBG1 );
				pcs_writel( p, BMCR_ANENABLE | BMCR_ANRESTART, MII_BMCR );
				pr_info("Port %d: RX calibration complete at phase %d ps (after %d attempts).\n", p->hw_index + 1, phase, fsm->attempts );
				printf("MDIO_MCR %04x\n", pcs_readl(p, 0) );
				rts_enable_ptracker( p->hw_index, 0 );
...
static int hal_port_init(int index)
{
	...
	error = libwr_cfg_convert2("PORT%02i_PARAMS", "monitor",
				   LIBWR_STRING, s, port_i);
	if (!error) {
		if (!strcasecmp(s, "n")) {
			p->monitor = HAL_PORT_MONITOR_DISABLE;
			pr_info("Port %s: monitor=n (%i)\n", p->name,
				p->monitor);
		} else if (!strcasecmp(s, "y")) {
			p->monitor = HAL_PORT_MONITOR_ENABLE;
			pr_info("Port %s: monitor=y (%i)\n", p->name,
				p->monitor);
		} else {
			/* error */
...
int hal_port_init_shmem(char *logfilename)
{
	...
	libwr_tmo_init(&link_led_blink_tmo, 200, 1);
...
static int hal_port_link_down(struct hal_port_state * p, int link_up)
{
	...
	if (!link_up 
		...
		rts_enable_ptracker(p->hw_index, 0);
		p->rx_setup_fsm->state = RX_SETUP_STATE_INIT;
...
static int tx_fsm_summary_displayed = 0;

static void tx_fsm_display_summary(void )
{
	int i;
	int busy = 0;
	...
	if (tx_fsm_summary_displayed )
		return;

	tx_fsm_summary_displayed = 1;

	pr_info("TX Calibration summary (FPGA temperature : %.3f degC)\n", (float)hal_get_fpga_temperature() / 256.0 );

	for (i = 0; i < HAL_MAX_PORTS; i++)
		if ( ports[i].in_use )
		{
			pr_info("port %d: TX phase = %d ps (after %d attempts)\n", ports[i].hw_index, ports[i].tx_setup_fsm->measured_phase, ports[i].tx_setup_fsm->attempts );
		}
}
...
static void set_led_wrmode(int p_index, int val)
{
	...
	} else if (val == SFP_LED_WRMODE_NO_LINK) {
...
static int link_led_blink_state = 0;
...
static void update_link_leds(void)
{
	...
	if( libwr_tmo_expired( &link_led_blink_tmo ) )
	{
		link_led_blink_state = 1 - link_led_blink_state;
	} 
	...
	for (i = 0; i < HAL_MAX_PORTS; i++) {
		...
		port_state = 0;
		if ( ppsi_state_valid && port_in_use )
		{
		...
		if( ports[i].in_use && ( 
			(ports[i].rx_setup_fsm->state != RX_SETUP_DONE && ports[i].rx_setup_fsm->state != RX_SETUP_STATE_INIT ) ||
			ports[i].tx_setup_fsm->state != TX_SETUP_DONE ) )
			{
				set_led_wrmode(i, link_led_blink_state ? SFP_LED_WRMODE_CALIBRATING : SFP_LED_WRMODE_NO_LINK );
			}
		else if( ports[i].state != HAL_PORT_STATE_UP)
			set_led_wrmode(i, SFP_LED_WRMODE_NO_LINK);
		else if (port_state == PPS_SLAVE)
