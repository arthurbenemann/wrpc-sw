...
static struct sdbf *scan_inputdir(char *name, struct sdbf *parent, FILE **cfgf)
{
	...
	n = 1;
	for (index=0;index < (tree->nfiles); index++) {
		/* dot is special: it fills slot 0 */
		if (!strcmp(namelist[index]->d_name, ".")) {
			tree[0].de = *(namelist[index]);
			__fill_dot(tree, name);
			continue;
		}
		if (!strcmp(namelist[index]->d_name, ".."))
			continue; /* no dot-dot */
		if (!strcmp(namelist[index]->d_name, CFG_NAME)) {
			char s[PATH_MAX];

			strcpy(s, name);
			strcat(s, "/");
			strcat(s, namelist[index]->d_name);
			*cfgf = fopen(s, "r");
			if (!*cfgf)
		...
		tree[n].de = *(namelist[index]);
		tree[n].dot = tree;
		ret = __fill_file(tree + n, name, namelist[index]->d_name);
		if (ret < 0)
			return NULL;
		n += ret;
	}
