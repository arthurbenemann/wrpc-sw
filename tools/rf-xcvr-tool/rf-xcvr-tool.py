...
class RFKinkySwappedBitdield(Bitfield):
    def __init__(self, owner, offset, width=1 ):
        super(RFKinkySwappedBitdield, self).__init__(owner, offset, width=width)
        if width == 1:
            if offset % 32 >= 16:
...
class RFMFrame(StreamerFrame):
    def __init__(self, streamers):
        super(RFMFrame, self).__init__(streamers)
        #print("plen: %d" % len(self.payload))
        self.control = Bitfield(self, 0, 16)
        self.rfm0 = RFKinkySwappedBitdield(self, 0+16, 32)
        self.rfm1 = RFKinkySwappedBitdield(self, 32+16, 32)
        self.rfm2 = RFKinkySwappedBitdield(self, 64+16, 32)
        self.rfm3 = RFKinkySwappedBitdield(self, 96+16, 32)
        self.rfm4 = RFKinkySwappedBitdield(self, 128+16, 32)
        self.rfm5 = RFKinkySwappedBitdield(self, 160+16, 32)
...
#print("CRCx = 0x%x" % frm.streamer_crc16(test_data) )

#frm.nco_reset.value = 1

while True:
    print("sendstrem\n")
    frm.tx_timestamp = 0xffff98e2 #fd90
    frm.rfm5.value = 0
    strm.send( frm )
    time.sleep(0.1)
    frm.rfm5.value = 1
