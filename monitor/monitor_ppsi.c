...
/*    cprintf(C_BLUE, "\n\nSync_method: two_step:");
    cprintf(C_GREY, "%d", WR_DSPOR(ppi)->ctr_twostep);
    cprintf(C_BLUE, ": one_step:");
    cprintf(C_GREY, "%d", WR_DSPOR(ppi)->ctr_onestep);
    cprintf(C_BLUE, "\n\nHas_faults:");
    cprintf(C_GREY, "%d", WR_DSPOR(ppi)->ctr_has_faults);
    cprintf(C_BLUE, ": is_incorrect_before:");
    cprintf(C_GREY, "%d", WR_DSPOR(ppi)->ctr_incorrect_before);
    cprintf(C_BLUE, ": is_incorrect_after:");
    cprintf(C_GREY, "%d", WR_DSPOR(ppi)->ctr_incorrect_after);
    cprintf(C_BLUE, "\n\nwr_s1:");
    cprintf(C_GREY, "%d", WR_DSPOR(ppi)->ctr_wr_s1);
    cprintf(C_BLUE, "\n\nbmc_go_to_slave:");
    cprintf(C_GREY, "%d", WR_DSPOR(ppi)->ctr_bmc_gotoslave);
*/
    cprintf(C_BLUE, "\n\next_specific: 0x");
...
int wrc_mon_gui(void)
{
    ...
    cprintf(C_GREY, "Servo_state:               ");
	cprintf(C_WHITE, "%s\n", s->servo_state_name);
	// TMP KM3 cprintf(C_GREY, "Phase tracking:            ");
    cprintf(C_GREY, "Phase_tracking:            ");
	...
	for(i = 0; i < n_out; i++) {
		...
		cprintf(C_GREY, "Aux_clock_%d_status:        ", i);
